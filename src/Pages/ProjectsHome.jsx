...
const ProjectsHome = () => {
  ...
  const ArticleCard = ({ article }) => (
    <a href={article.category} target="_blank" className="relative border group h-[500px] lg:h-[750px]  p-2 lg:p-5 border-white/10 ">
      
            
      <div  className="relative h-[400px] sm:h-[350px] group  lg:h-[600px] overflow-hidden shadow-lg">
        <div className="absolute z-50 inset-0 bbg-[#020617] opacity-0 group-hover:opacity-30 transition-all duration-300 "/>
      ...
      <div className=" w-full flex flex-col gap-5 items-center justify-center">
        ...
        <div className="grid grid-cols-4 w-full">
            {skills.map((skill,i)=>{
            return(
                <h5 className="h-6 flex justify-center items-center   text-gray-300 border border-white/10">{skill}</h5>
            )
        })}
        </div>
  ...
  return (
    <ReactLenis root>
      <div className="">
        ...
        {/* ArticlesSection */}
        <section className="w-full  bg-[#020617] text-white p-12 ">
            <div className="mb-12">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
          <h1 className="text-6xl md:text-9xl  font-[font1] text-lime-400">
            BEST PROJECTS
          </h1>
          <div className="text-lg leading-[0.95] text-gray-400 max-w-xs pt-2 font-[font2]">
            A showcase of standout builds—where design meets performance. Explore the projects that define my craft and creativity.
          </div>
        </div>
        {/* <div className="flex flex-wrap gap-2 md:gap-4 mt-8">
          {tags.map((tag) => (
            <div
              key={tag}
              className="border border-gray-700 text-gray-400 rounded-full px-4 py-1 text-sm flex items-center gap-2 cursor-pointer hover:bg-gray-800 transition-colors"
            >
              <PlusCircleIcon />
              <div>{tag}</div>
            </div>
